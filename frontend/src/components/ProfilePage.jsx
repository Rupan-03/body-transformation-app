...
import {
  User, Calendar, Ruler, Weight as WeightIcon, Activity as ActivityIcon, Target,
  Save, Loader2, Venus, Mars, TrendingUp, TrendingDown, ArrowLeft
} from 'lucide-react';
...
export default function ProfilePage({ onProfileSave, onBackToAuth }) {
  ...
  // convert number-like strings to numbers to match API expectations
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Pass back either {user: ...} or the user object—App handles both
      const savedUser = res.data?.user || res.data;
      onProfileSave(savedUser);
    } catch (err) {
  ...
  const handleBack = () => {
    ...
    // Fallback: ensure the app shows Auth
    try {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 flex items-center justify-center p-4">
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="w-full max-w-2xl">
        <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border border-slate-200/60 overflow-hidden">
          {/* Header */}
          <div className="relative bg-gradient-to-r from-blue-500 to-blue-600 text-white p-8 text-center">
            {/* Back to Auth button */}
            <button
              type="button"
              onClick={handleBack}
              className="absolute left-4 top-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/20 hover:bg-white/30 backdrop-blur-sm text-white text-sm font-medium transition-colors"
              ...
              <motion.section variants={itemVariants}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      >
                        ...
                        <span className={`font-medium ${formData.gender === 'male' ? 'text-blue-700' : 'text-slate-600'}`}>
                          Male
                        </span>
                      </button>
                      <button
                      ...
                      >
                        ...
                        <span className={`font-medium ${formData.gender === 'female' ? 'text-pink-700' : 'text-slate-600'}`}>
                          Female
                        </span>
                      </button>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <WeightIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
                      <input
                        name="weight"
                        type="number"
                        min="0"
