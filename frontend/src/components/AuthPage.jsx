...
import { AuthFormSkeleton } from './AppSkeletons'; // ⮕ skeleton for form panel
...
export default function AuthPage({ onAuthSuccess, onNavigate }) {
  ...
  const [panelLoading, setPanelLoading] = useState(true); // ⮕ skeleton state for panel
  const [shake, setShake] = useState(false);

  const timeoutsRef = useRef([]);

  // hydrate the form with a tiny skeleton on first mount
  useEffect(() => {
    const t = setTimeout(() => setPanelLoading(false), 280);
  ...
  // ⮕ Small helper to show skeleton when switching tabs
  const switchTab = (nextIsRegister) => {
    if (isRegister === nextIsRegister) return;
    setPanelLoading(true);
    setIsRegister(nextIsRegister);
    const t = setTimeout(() => setPanelLoading(false), 220);
    timeoutsRef.current.push(t);
  };
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="w-full max-w-md mx-auto">
          <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border border-slate-200/60 overflow-hidden">
            ...
            <div className="p-8">
              ...
              {/* ⮕ Panel skeleton when switching / initial load */}
              {panelLoading ? (
                <AuthFormSkeleton />
              ) : (
                  ...
                  <motion.div key={isRegister ? 'register' : 'login'} variants={panelVariants} initial="hidden" animate="visible" exit="exit">
                    ...
                    <motion.div variants={itemVariants} className="mt-6 text-center">
                      <p className="text-slate-600">
                        ...
                        <button onClick={() => switchTab(!isRegister)} className="font-semibold text-blue-600 hover:text-blue-700 transition-colors">
